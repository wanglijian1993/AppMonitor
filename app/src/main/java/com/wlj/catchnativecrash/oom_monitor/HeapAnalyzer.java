 ...
 */
class HeapAnalyzer {
    private LinkedHashSet<Instance> toVisitSet=new LinkedHashSet();
    public void analysisLeak(String key) throws IOException {
        // 怎么样找到泄漏链
        // 1. dump 生成一个 hpro 文件
        File heapDumpFile = new File(MyApplication.application.getBaseContext().getFilesDir(), "test.hprof");
        Log.e("HeapAnalyzer","file:"+heapDumpFile.getAbsolutePath());
